...
class class_db {
    ...
    function connect($db_data){       
        ...
        if ($this->db_type == "sqlite"){
        	...
        	if (!$this->db_link = new SQLite3('dfsindex_db')) {    
        		//$this->db_link->query("PRAGMA count_changes=0");
        		//$this->db_link->query("PRAGMA synchronous=OFF");
        		//$this->db_link->query("BEGIN TRANSACTION");
        		throw new RuntimeException("Connect failed: %s\n". $sqliteerror);
        	}
    ...
    public function runquery($string)
    {
       ...
       if (!$this->result) {          
           throw new Exception('SQL Anweisung: '.$string.' konnte nicht durchgef&uuml;hrt werden. Fehler: '.$this->db_link->lastErrorMsg());
    ...
    public function trucateTable($table){
     ...
     public function ReadRow() {
        ...
        if ($this->RowData = sqlite_fetch_array($this->result)) {
        	return 1;
        } else {
        	return 0;
        }
